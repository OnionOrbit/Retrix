			   ...
			   <div class="mt-auto text-secondary text-sm">
				   ...
				   <div class="flex items-center gap-3">
					   <button
						   ...
						   @click="devModeCount"
					   >
						   <!-- Optionally replace with a Retrix icon if available -->
						   <SettingsIcon class="w-6 h-6" />
					   </button>
					   <div>
						   <p class="m-0">Retrix {{ version }}</p>
						   <p class="m-0">
							   <span v-if="osPlatform === 'macos'">MacOS</span>
							   <span v-else class="capitalize">{{ osPlatform }}</span>
							   {{ osVersion }}
						   </p>
						   <p class="m-0 mt-2 text-xs text-warning">
							   Retrix is an unofficial fork of the Modrinth Launcher. Not affiliated with Modrinth, Mojang, or Microsoft.
						   </p>
					   </div>
				   </div>
			   </div>
